...
public class FrontController {
        ...
        if (authentication.isAuthenticated()) {
            ...
            if (u.getChannel() != null) {
                for (Channel channel : new ArrayList<>(u.getChannel())) {
                    ...
                    // System.out.println(channel);
                    simpMessagingTemplate.convertAndSend("/topic/exit" + channel.getChannelname(),
                            authentication.getName() + " has left the channel.");
    ...
    public ResponseEntity<String> deleteChannel(@RequestBody Map<String, String> payload,
            Authentication authentication) {
    ...
    // @PostMapping("/share")
    // public String shareChannel(@RequestParam("channelname") String name, Model
    // model) {
    // Channel channel = channelRepo.findById(name).orElseThrow();
